...
import { getAdjacentPosts } from "@/lib/posts/queries";
...
// getCachedPostById already shares a result across requests (Next's Data
// Cache — see lib/posts/cached.ts); wrapping it in React's cache() on top of
// that means generateMetadata and the page body share one call *within this
// request* too, instead of each asking the Data Cache separately.
const getCachedPost = cache((postId: number) => getCachedPostById(postId));
...
/**
 * The prev/next links: keyboard nav plus the mobile/desktop footer nav.
 *
 * Split out and streamed in behind its own Suspense boundary so the post's
 * own title and body — including the LCP image — don't wait on this extra
 * `getAdjacentPosts` query. It runs concurrently with the rest of the page
 * instead of after it, and the shell flushes as soon as the post row itself
 * is ready.
 */
async function AdjacentPostNav({
  postId,
  postDate,
  userID,
}: {
  ...
  const { newer, older } = await getAdjacentPosts({ id: postId, post_date: postDate, user_id: userID });

  return (
    <>
      <PostKeyboardNav
        key={`keyboard-nav-${postId}`}
...
export default async function Page({
  ...
  <main id="main-content" className="mx-auto w-full max-w-prose p-4 pb-24 text-foreground md:pb-4">
    <div className="text-center">
      ...
      <p className="text-muted-foreground">{new Date(post.post_date).toLocaleDateString(undefined, { timeZone: "UTC" })}</p>
      ...
      {isAuthor && (
        <div className="mt-4 flex items-center justify-center gap-5">
          <Link
            ...
            className="inline-flex items-center gap-1.5 text-sm text-muted-foreground underline-offset-4 transition-colors hover:text-foreground hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-muted-foreground"
          >
    ...
    {/* Streamed in behind the post body: the title and body (including the
        LCP image) flush as soon as the post row is ready, and this footer's
        own `getAdjacentPosts` query no longer has to finish first. */}
    <Suspense fallback={null}>
      <AdjacentPostNav postId={post.id} postDate={post.post_date} userID={userID} />
    </Suspense>
